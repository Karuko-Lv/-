...
$q 1$ 包含 $a 1$ 的信息，$k 1$ 包含 $a 1$ 的信息，$k 2$ 包含 $a 2$ 的信息... $\alpha1,1$ 、$\alpha 1,2$ 等是用来计算第一个输入的词和其他词之间的关联程度，通过softmax 函数做映射，又softmax 函数输出来的结果相加等于 1，所以最后的 $\alpha'1,1$ 可以表达 $a 1$ 和自己的关联程度，$\alpha'1,2$ 可以表达 $a 1$ 和a 2 的关联程度... 关键是找到一组合适的W 矩阵参数矩阵![[截屏2026-01-19 14.26.13.png]]
🧚‍♀️词向量矩阵＋位置编码