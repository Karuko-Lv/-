...
**自注意力机制的算法模型**
1. 计算输入数据 $x$ 之间的关联程度
2. 利用关联程度提取输入数据 $x$ 的有用信息
3. 将有用信息融合到一起做输出，输出包含不同注意力分配的信息 

输入的 $a 1 a 2 a 3 a 4$ 是自然语言数据，转换成词向量矩阵（嵌入矩阵）的数值进行模型计算，分别对向量矩阵 $a 1 a 2 a 3 a 4$ 进行模型权重的相乘，进行映射
![[截屏2026-01-19 14.21.58.png]]
$q 1$ 包含 $a 1$ 的信息，$k 1$ 包含 $a 1$ 的信息，$k 2$ 包含 $a 2$ 的信息... $\alpha1,1$ 、$\alpha 1,2$ 等是用来计算第一个输入的词和其他词之间的关联程度，通过softmax 函数做映射，又softmax 函数输出来的结果相加等于 1，所以最后的 $\alpha'1,1$ 可以表达 $a 1$ 和自己的关联程度，$\alpha'1,2$ 可以表达 $a 1$ 和a 2 的关联程度... 关键是找到一组合适的W 矩阵（参数矩阵）使得最后的映射结果是关联程度，W 是通过反向传播获取的，也就是transformer 的模型架构是通过模型训练，然后利用标签进行loss 值的计算再进行反向传播去更新W 的![[截屏2026-01-19 14.26.13.png]]
🧚‍♀️词向量矩阵＋位置编码，我觉得还是得深入了解一下 QKV 分别代表什么
🤨QKV 中的 V 是每个 key 对应的 value 矩阵，类似于这个 key 的特征向量（即数据的特征表示）
$K = X \cdot W_K$ ，$V = X \cdot W_V$
- $W_K$ 是一个参数矩阵，专门负责把 $X$ 变成“用来匹配的样子”。
- $W_V$ 是另一个参数矩阵，专门负责把 $X$ 变成“用来传递内容的样子”。
它们是**平行**产生的，互不干扰。$K$ 长什么样，理论上不影响 $V$ 长什么样。