...
### 极简神经网络
通过这个过程，你会发现那些复杂的 AI 算法，本质上就是这几行乘法运算。
...
假设我们有一个最简单的神经元：
- **输入 ($x$):** $1.0$
- **真实标签 ($y$):** $0.0$ （我们希望输出越接近 0 越好）
- **权重 ($w$):** 初始设为 $0.8$
- **偏置 ($b$):** 初始设为 $0.1$
- **激活函数 ($\sigma$):** 我们使用 Sigmoid 函数，其导数有个很漂亮的特性：$\sigma'(z) = \sigma(z)(1 - \sigma(z))$。
- **损失函数 ($L$):** 使用平方误差 $L = \frac{1}{2}(\hat{y} - y)^2$。

---
#### 2. 前向传播 (Forward Pass)

第一步，先算出当前的预测值是多少：
1. 加权求和 ($z$):$$z = w \cdot x + b = 0.8 \cdot 1.0 + 0.1 = 0.9$$
2. 激活输出 ($a$ 或 $\hat{y}$):
    假设 $\sigma(0.9) \approx 0.71$。所以现在的预测值 $\hat{y} = 0.71$。
3. 计算损失 ($L$):$$L = \frac{1}{2}(0.71 - 0.0)^2 \approx 0.25$$
---

#### 3. 反向传播 (Backpropagation)

现在我们要计算：**如果要减小损失 $L$，该如何调整 $w$ 和 $b$？** 根据**链式法则**，我们从后往前“剥洋葱”：
##### 第一层：损失对输出的导数（误差有多大？）

$$\frac{\partial L}{\partial a} = (a - y) = 0.71 - 0.0 = \mathbf{0.71}$$

> **物理意义：** 预测值比目标值大了 0.71。

##### 第二层：输出对加权和的导数（激活函数的敏感度）

$$\frac{\partial a}{\partial z} = \sigma'(z) = a(1 - a) = 0.71 \times (1 - 0.71) \approx \mathbf{0.20}$$

> **物理意义：** 当前神经元处于比较敏感的状态，调整输入会对输出产生约 0.2 倍的影响。

##### 第三层：加权和对参数的导数
1. **对权重 $w$：** $\frac{\partial z}{\partial w} = x = \mathbf{1.0}$
2. **对偏置 $b$：** $\frac{\partial z}{\partial b} = \mathbf{1.0}$
---
...
$$\frac{\partial L}{\partial w} = \frac{\partial L}{\partial a} \cdot \frac{\partial a}{\partial z} \cdot \frac{\partial z}{\partial w} = 0.71 \times 0.20 \times 1.0 = \mathbf{0.142}$$

偏置 $b$ 的梯度：

$$\frac{\partial L}{\partial b} = \frac{\partial L}{\partial a} \cdot \frac{\partial a}{\partial z} \cdot \frac{\partial z}{\partial b} = 0.71 \times 0.20 \times 1.0 = \mathbf{0.142}$$

> **注意：** 看到没？因为输入 $x=1.0$，所以在这个特殊的例子里，$w$ 和 $b$ 的梯度一模一样！正如你之前猜想的，**偏置就像一个输入永远为 1 的权重。**

---

#### 5. 参数更新 (Gradient Descent)

假设学习率 $\eta = 0.1$，我们来更新参数：
- **新权重：** $w_{new} = w - 0.1 \times 0.142 = 0.7858$
- **新偏置：** $b_{new} = b - 0.1 \times 0.142 = 0.0858$
结果分析：
权重和偏置都减小了。因为我们的目标是让输出从 $0.71$ 降到 $0$，所以把参数调小是非常合理的。
...
1. **算出误差**（最后一层）。
2. **乘上激活函数的斜率**（中间层）。
3. **乘上对应的输入值**（最前层，如果是 $b$ 就乘 1）。

**想一想：如果我们的输入 $x$ 变成了 $0$，按照上面的公式，权重的梯度 $\frac{\partial L}{\partial w}$ 会变成多少？这是否印证了你之前说的“$w$ 为 0 时信息被屏蔽”的逻辑？**