...
3. DFT ：有限长傅立叶变换
	...
	8. 利用 DFT 分析连续信号频谱 **(核心难点)**
		0. 一些知识点：
			1. 信号之间的关系 ：离散即抽样，有限长信号 $x(t)$ 抽样变成有限长序列 $x[k]$ $$\begin{array}{ccccc}x(t) & \xrightarrow{\hspace{1em}\text{离散化}\hspace{1em}} & x[k] & \xrightarrow{\hspace{1em}\text{周期化}\hspace{1em}} & \tilde{x}[k] \\\Big\downarrow & & \Big\downarrow & & \Big\downarrow \text{\small DFT} \\X(\mathrm{j}\omega) & \xrightarrow{\hspace{1em}\text{周期化}\hspace{1em}} & X(\mathrm{e}^{\mathrm{j}\Omega}) & \xrightarrow{\hspace{1em}\text{离散化}\hspace{1em}} & \tilde{X}[m] \end{array} $$
				1. 抽样定理（频域混叠） $X(e^{j\Omega}) = \frac{1}{T} \sum_{n=-\infty}^{\infty} X[j(\omega + n\omega_{sam})]$ 
					- $x[n] = x(t) \Big|_{t=nT} = x(nT)$，这里的 $T$ 就是采样周期
		...
		1. 重要公式：
			...
			2. **$\Omega$** (有时写作 $\hat{\omega}$ 或 $\theta$)（数字角频率）：每采一个点，相位转了多少度；~={red}**归一化**=~频率 ~={red} $\Omega = \omega \cdot T_{sam}=2\pi\frac{f}{f_{sam}}=2\pi\frac{w}{w_{sam}}$ =~ $T_{sam}$ 即两个采样点之间的时间间隔 🧚‍♀️非阻塞性赋值 $T\leq \frac{1}{f_{sam}}$ <== 非阻塞赋值，只有到最终的时候才会显现出赋的值，看这个课的电信同学都用这个吗
		...
		3. 泄漏现象：$$x(t) \xrightarrow{\color{green}{\text{抽样}}} x[k] \xrightarrow{\color{red}{\text{加窗}}} x_N[k] \xrightarrow{\color{purple}{\textbf{DFT}}} X[m]$$
			- 生活中永远也不可能得到无限长的正弦信号或余弦信号，分析正弦和余弦信号的频谱，谱的性质不是冲激函数而是周期延拓的 Sa 函数![[截屏 2025-12-01 12.18.25.jpeg]]
			- **原因**：时域截断（加窗） $x_N[k]=x[k]w_N[k]$
			    ...
			    - 窗函数的**旁瓣**导致能量泄漏到其他频率。
			    - $N \uparrow$ 增加-->主瓣宽度减小 $\downarrow$ -->泄漏（的范围）降低 $\downarrow$ ，但是泄露依然存在🧚‍♀️矩形窗时域展宽频域变窄
			...
			- 加权窗：矩形窗函数每一点的值都是 1，加权之后每一点的值不都等于 1，轮廓大致相同，主瓣和旁瓣有差异
			    1.  **汉宁窗 (Hanning)**：对矩形窗进行加权，主瓣变宽 ($4\pi/N$)，旁瓣幅度降低 (-31dB)。分辨率低，但泄漏少。
				    1. 公式：$\color{#5e2526}{w_N[k] = \left[ \frac{1}{2} - \frac{1}{2} \cos \left( \frac{2\pi}{N-1} k \right) \right] R_N[k]} = \color{blue}{ \begin{cases} 0.5 - 0.5 \cos(2\pi k / (N-1)) & 0 \le k \le N-1 \\ 0 & \text{others} \end{cases} }$
				    2. 特点：时域变换比较平缓，不是突然从 1 变为 0==>频域即窗函数对应的频谱中的幅度谱，主瓣宽度变宽、旁瓣幅度降低
				    3. 主瓣有效宽度：$\Delta\Omega_w = \frac{4\pi}{N}$ -->比矩形窗的主瓣有效宽度展宽了一倍
				    4. 旁瓣的峰值衰耗：$A \approx 31 \text{ dB}$，比矩形窗小得多，原因是对矩形窗加权之后时域变得平缓，不是突然截短，所以高频分量相应降低🧚‍♀️这时域越来越接近一个余弦函数了，频域应该越来越接近冲击
				2. **汉明窗/海明窗 (Hamming)**：对矩形窗进行加权，主瓣变宽 ($8\pi/N$)，旁瓣幅度降低 (-41dB)。分辨率低，但泄漏少。
					1. 公式：$w_N[k] = \left[ 0.54 - 0.46 \cos \left( \frac{2\pi}{N-1} k \right) \right] R_N[k] = \begin{cases} 0.54 - 0.46 \cos(2\pi k / (N-1)) & 0 \le k \le N-1 \\ 0 & \text{others} \end{cases}$
					2. 主瓣有效宽度：$\Delta\Omega_w = \frac{4\pi}{N}$ -->比矩形窗的主瓣有效宽度展宽了一倍
					3. 旁瓣的峰值衰耗：$A \approx 41 \text{ dB}$ -->旁瓣的峰值衰耗进一步降低，更加光滑
				3. **布莱克曼窗/黑人窗（Blackman）**：
					1. 公式：$w[k] = \left[ 0.42 - 0.5 \cos \left( \frac{2\pi}{N-1} k \right) + 0.08 \cos \left( \frac{4\pi}{N-1} k \right) \right] R_N[k]$
					2. 主瓣有效宽度：$\Delta\Omega_w = \frac{6\pi}{N}$ -->主瓣有效宽度进一步展宽，旁瓣的幅度进一步降低
					3. 旁瓣的峰值衰耗：$A \approx 57 \text{ dB}$ -->旁瓣的峰值衰耗进一步降低，高达 $57 dB$
				4. **凯塞窗/可调窗 (Kaiser)**
					1. 公式：$w_N[k] = \frac{I_0 \left( \beta \sqrt{1 - (1 - 2k/N)^2} \right)}{I_0(\beta)} R_N[k]$ ($\beta = 5.86$)
						PS：调整参数 $\beta$ 可以得到相应的不同的时域波形的窗函数
					2. **主瓣有效宽度：** $\Delta\Omega_w = \frac{5\pi}{N}$ 
					3. 旁瓣的峰值衰耗：$A \approx 57 \text{ dB}$ 
			    【总结】矩形窗主瓣有效宽度定义为 $2\pi/N$，而汉明窗/汉宁窗通常认为是 $4\pi/N$（基于零点或主瓣宽度计算方式不同，注意课件中的 $\Delta\Omega_w$ 定义）
			    ![[Pasted image 20251201141409.png]]
			    常用窗函数特性：主瓣宽度变宽，相应旁瓣峰值衰耗也会增加，窗函数进行 DFT 频谱分析时，合理选择相应的窗函数，使得频谱分析更加有效![[截屏2025-12-01 14.15.19.png]]
			- **对策**：
			    1. 增加截取长度 $N$（使主瓣变窄，提高分辨率）。
			    2. 选择旁瓣衰减大的非矩形窗（如 Hamming, Blackman）
			- 题目：
				1. 对一个连续信号进行 DFT 分解（找出复杂的波形，到底是由哪些简单的正弦波（频率、幅度和相位）叠加而成的）求频谱，首先要将其变成离散信号（抽样）
					1. 理想采样公式：$x[k] = x(t) \Big|_{t=kT_{sam}} = x(kT_{sam})=x(\frac{k}{f_{sam}})$ 时间无限长频带有限的
					2. 加窗截短（先加矩形窗）：$x_N[k]=x[k]w_N[k]$ --> $X_{N}(e^{j\Omega}) = X(e^{j\Omega}) * W(e^{j\Omega})$ 
				2. 矩形窗口宽度 $N$ 的选择对能够分辨出 $x[k]$ 中是否含有 $f_1$, $f_2$ 这两个频率点有何影响（已知某连续信号为 $x(t)=\cos(2\pi f_1t)+\cos(2\pi f_2t)$， $f_1=100\text{Hz}$，$f_2=120\text{Hz}$ 若以抽样频率 $f_{\text{sam}}=600\text{Hz}$ 对该信号进行抽样，试计算由 DFT 分析其频谱时，能够分辨此两个谱峰所需的最少样本点数） ![[Pasted image 20251201145542.png]]
					1. ⚠️（做题时默认）矩形窗口宽度和抽样样本数 $N$ 是一个东西：采集了 **$N$** 个点相当于你给信号加了一个长度为 **$N$** 的矩形窗，**$N$** 决定了窗函数的频谱主瓣宽度（$\Delta \Omega = \frac{2\pi}{N}$）-->想要分辨两个靠得很近的频率，主瓣须足够窄-->需要增加采集的点数 $N$
					2. 想要分辨两个频率，希望谱峰之间分布如下，如果 $\Delta \Omega_w$ 太宽，谱峰会重叠，所以 $\color{blue}{\Delta\Omega_{\text{w}} \le \Delta\Omega / 2} \quad \implies \quad \frac{2\pi}{N} \le \frac{\pi\Delta f}{f_{\text{sam}}}$$$\color{#6a5acd}{\text{矩形窗口宽度}} \quad \color{#8b0000}{\boxed{ N \ge \frac{2 f_{\text{sam}}}{\Delta f} = \frac{2 f_{\text{sam}}}{f_2 - f_1} }}$$ (🧚‍♀️分辨率够小，才能分辨得出来🐤两个频率之间的距离，必须至少是主瓣宽度的 2 倍)   $N \uparrow$ 越大-->主瓣宽度越窄 $\downarrow$ -->谱峰的分辨越清晰  ![[截屏2025-12-01 15.09.57.png|300]]
					3. $N$ 进一步 $\downarrow$ -->主瓣宽度变宽 $\uparrow$ --> $N=\frac{f_{sam}}{f_2-f_1}$，也可以分辨出两个谱峰，但是由于谱峰的位置混叠在一起（有交叠的部分）谱峰的位置是不准确的🧚‍♀️妙啊，混叠造成了波峰偏移；伪峰 ![[截屏2025-12-01 15.23.31.png|300]]
					4. 如果两个余弦信号幅度相差较大（已知某连续信号为 $x(t)=\cos(2\pi f_1t)+0.15\cos(2\pi f_2t)$， $f_1=100\text{Hz}$， $f_2=150\text{Hz}$。若以抽样频率 $f_{\text{sam}}=600\text{Hz}$ 对该信号进行抽样，试计算由DFT分析其频谱时，能够分辨此两个谱峰所需的最少样本点数）
						1. 如果还取两个谱峰之间矩形框主瓣刚好没有交叉的时候 $\color{blue}{\Delta\Omega_{\text{w}} = \Delta\Omega / 2} \quad \implies \quad \frac{2\pi}{N} = \frac{\pi\Delta f}{f_{\text{sam}}}$ ，$N = \frac{\color{red}{2}\color{blue}{f_{\text{sam}}}}{f_2 - f_1} = \frac{\color{red}{2}\color{blue}{f_{\text{sam}}}}{\Delta f} = \color{blue}{\frac{2 \times 600}{50} = 24}$ ->100 和 120 两个点分不出==>因为矩形窗的旁瓣幅度较大，第一个信号旁瓣的幅度可能比第二个信号的主瓣幅度还高或者相当，所以看不出有两个频率 ![[截屏2025-12-01 15.57.38.png|300]]
						2. 如果加一倍 $N=48$ 也还不是很理想，由于我们有多种窗函数所以可以考虑换一种窗-->换一个旁瓣幅度较小的窗函数，例如：海明窗，主瓣有效宽度等于两个频率之差的一半 $\color{blue}{\Delta\Omega_{\text{w}} = \Delta\Omega / 2}$  $\Delta\Omega_w = \frac{4\pi}{N}$ 来选择 $N = \frac{\color{red}{4}\color{blue}{f_{\text{sam}}}}{f_2 - f_1} = \frac{\color{red}{4}\color{blue}{f_{\text{sam}}}}{\Delta f} = \color{blue}{\frac{4 \times 600}{50} = 48}$ ![[截屏2025-12-01 16.07.09.png]]
						3. 【总结】窗口类型选择
							1. 期望的窗函数：
								1. 主瓣宽度越小越好
								2. 旁瓣衰耗越大越好
							2. 实际的窗函数：（有得必有失）
								1. 主瓣窄、旁瓣衰耗小
								2. 主瓣宽、旁瓣衰耗大，意味着旁瓣**很低**（弱）。
							【心得🤨】对于信号幅度差别大的，要尽可能让旁瓣衰耗大，意味着旁瓣**很低**（弱），主瓣宽度就会越宽，例如布莱克曼窗旁瓣最低（最好），但主瓣**最宽**（$6\pi/N$）。
						4. 频谱分辨率(DFT 的“刻度尺”)：$\Delta \omega_c=\Delta \Omega_w / T=\frac{2\pi}{NT} = \underbrace{\left( \frac{2\pi}{N} \right)}_{\text{数字频率}} \times \underbrace{\left( \frac{1}{T} \right)}_{\text{换算系数}}$ (模拟频率和数字频率)==> $\Delta f_c = \Delta \omega_c / 2\pi = \frac{1}{NT}$，$\Delta f_c = \frac{1}{T_{total}}$（频率和模拟频率）（实际工程中到现场去信号实际上是无形中对这个信号加一个矩形窗，通过矩形窗引入频谱分辨率的概念 ）🧚‍♀️频谱分辨率就是频域最小间隔（在大多数基础教科书和考试中，通常把它们混为一谈（视为同一个）；但在严谨的工程实践中，它们是两个完全不同的概念）
							1. **尺子的总长度：** 采样频率 $f_{sam}$（能测到的最高频率范围）。
							2. **尺子的刻度数：** 采样点数 $N$。
							3. 最小刻度值 ($\Delta f_c$)：$\text{最小刻度} = \frac{\text{总长}}{\text{刻度数}} = \frac{f_{sam}}{N}$ , $f_{sam} = 1/T$, $\Delta f_c = \frac{1/T}{N} = \frac{1}{NT}$
						5. 采集信号时间（时间分辨率）：$T_p = NT = \frac{1}{\Delta f_c}$ 🧚‍♀️ $T$ 是抽样时间，$T_p$ 是N点总时间$T_{total}$
		4. 栅栏现象 zhalan🧚‍♀️栅极源极漏极
			- 介绍： $\begin{matrix} X[m] = X_N(e^{j\Omega}) \big|_{\Omega = \frac{2\pi}{N}m} \\ \downarrow \\ x_N[k] \end{matrix},m=0,1..,N-1$， $X_N(e^{j\Omega})$ 是 $x_N[k]$ 的真实频谱
				- $DFT$ 是用来分析我们前面对应的四种信号频谱的一种方法，基本原理是从前面四种信号频谱延伸过来的，只有 $X_N(e^{j\Omega})$ 是 $x_N[k]$ 的真实频谱，本来要求 $x_N[k]$ 的频谱，但是现在得不到，而是通过求 $x[m]$ 的 DFT，即 $X_N(e^{j\Omega})$ 在一个周期内的等间隔抽样。要的是连续谱，得到的是连续谱一个周期内的样本值，类似于隔着栅栏看东西
				- **原因**总结：DFT 只是频谱的离散采样。就像透过栅栏看风景，可能漏掉栅栏（采样点）之间的频谱峰值。
			- **谱线间隔**：$\Delta f_d = \frac{f_{sam}}{N}(Hz)$ 换算到模拟这个轴，即 $f_{sam}$ 是在一个周期（一个周期内取了 N 个点，所以谱线间隔用频率表示）
				- 理解： $\underbrace{2\pi}_{\text{数字周期}} \iff \underbrace{f_{sam}}_{\text{模拟周期}}$，$\underbrace{\frac{2\pi}{N}}_{\text{数字间隔}} \iff \underbrace{\frac{f_{sam}}{N}}_{\text{模拟间隔}}$DFT 把这一个周期的跨度均分成了 $N$ 份，所以每一份的频率间隔就是 $f_{sam}/N$
			- **对策：补零**
				- 公式：$x_L[k] = \begin{cases} x_N[k] & 0 \le k \le N-1 \\ 0 & N \le k \le L-1 \end{cases}$  
					- 补零后的谱线间隔：$\Delta f_d = \frac{f_{\text{sam}}}{L}(\text{Hz}), \quad L > N$ 
					- 间隔小一些，谱线多一些
					- $N$ 是窗函数长度 
			    - **【考点】**：补零**不能**提高物理分辨率（分辨两个靠得很近的频率的能力），物理分辨率只取决于由于信号采集时长 $T_p$。🧚‍♀️补 0绝对没有增加观测时间，也绝对没有增加物理分辨率。
			    - 序列补零只能提高**显示分辨率**（谱线更密），让我们看到信号原来真实谱 $X(e^{j\Omega})$ 频谱的更多细节（样点值），减小栅栏效应。
			    - $N$ 点序列 $N$ 点DFT完全可以恢复信号 $x[k]$（相互一一对应可以恢复），为了在一个周期内观测到更多真实谱的样点值，可以在后面补0-->减少/改善栅栏现象，越来越能够看到原来信号连续谱的轮廓
4. 参数选择与计算公式 (计算题核心)：$x(t) \xrightarrow{\color{red}{\text{抗混叠}}} x_0(t) \xrightarrow{\color{green}{\text{抽样}}} x[k] \xrightarrow{\color{red}{\text{加窗}}} x_N[k] \xrightarrow{\color{purple}{\textbf{DFT}}} X[m]$ 
			1. 重要参数： $\begin{array}{c|c|c} \color{blue}{\textbf{频谱混叠}} & \color{blue}{\textbf{频谱泄漏}} & \color{blue}{\textbf{栅栏现象}} \\ \hline \color{purple}{\text{抽样频率:}} & \color{purple}{\text{采集时间:}} & \color{purple}{\text{DFT点数:}} \\ f_{\text{sam}} \ge 2 f_m & T_p = NT \ge \frac{1}{\Delta f_c} & L \ge \frac{f_{\text{sam}}}{\Delta f_d} \end{array}$
			2. 抽样频率：抽样定理中对一个带限信号，抽样频率受到最高频率的制约
			3. 采样时间：抽样频率知道，抽样间隔也知道了，抽样时间为 $T=\frac{1}{f_{sam}}$，窗口宽度 $N$ 也是一个关键参数
			4. 序列补 0 减少 $\Delta f_d$ 的值，减少样点之间的频谱间隔
			【总结】要重点掌握选取这些参数，在进行频谱分析时，参数选择至关重要
	1. **最高频率** $f_m$：决定采样频率。
		- $f_{sam} \ge 2 f_m$ 
	2. **频率分辨率** $\Delta f_c$ (Resolving Power物理分辨率)：分辨两个频率分量的能力，是能分辨两个靠得很近的频率峰值的**物理极限**
	    ...
	    - 类比：如果录音录得太短（$T_p$ 小），哪怕你用再高级的显微镜（DFT），你也看不清两个频率的区别，它们会糊在一起。
	3. **谱线间隔** $\Delta f_d$ (Display Resolution频率采样间隔)：DFT 输出点之间的频率间隔。
	    - 取决于 **DFT 的点数 $L$**（包含补零）$\Delta f_d = \frac{f_{sam}}{L}$ （其中 $L$ 是 DFT 计算点数，可能大于数据长度 $N$，即补零后）。
	    - 若要求观察细节，需 $\Delta f_d$ 足够小
	    - 这只是把曲线描得更细了，并没有提高视力。
	4. 观测时间 $T_p$：$T_p = N \times T$  观测时间越长 $T_p\uparrow$，分辨率越高，越能分清两个不同的频率； $T\downarrow$
	5. 抽样点数和 DFT 点数：🧚‍♀️个人理解，抽样点数是连续信号离散化时的点数，dft 点数是对加窗后离散序列 dft 变换时的点数；简单来说 L 是补零之后的点数；N 是采集那里窗函数的点数吧，然后 L 的 DFT 的补零后点数![[截屏2025-12-01 17.35.47.png]]
 【总结公式表】
